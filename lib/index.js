...
function dshCliCommands(pkg) {
  const name = pkg?.name || 'your-plugin'
  return [
    '## DSH 安装 / 卸载',
    '',
    '### 安装',
...
function buildReadme(dir, pkg, tree) {
  ...
  const isDsh = Boolean(pkg?.dsh || pkg?.name?.startsWith('@') || tree.some((line) => line.includes('lib/') || line.includes('src/')))
  const lines = [
    `# ${name}`,
    '',
    description,
...
function ensureReadme(dir, forceRewrite = false) {
  ...
  if (isDsh && !existing.includes('dsh plugin --profile web add')) missing.push('dsh-cli')
  ...
  const sections = missing.map((section) => {
    switch (section) {
      ...
      case 'dsh-cli': return dshCliCommands(pkg)
      default: return ''
    }
  }).filter(Boolean)
